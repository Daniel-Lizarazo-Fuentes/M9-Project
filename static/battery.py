...
class Battery():
    ...
    def verify_input(self):
        ...
        if self.batpmin < self.batcapacity * cfg_sim['bat_maxcrate'] * -1000:
            print("Input for battery " + str(
                self.number) + " is invalid! The minimum power (batpmin) may not be lower than -" + str(
                cfg_sim['bat_maxcrate']) + " C-rate. The lowest allowed value given its capacity is: " + str(
                self.batcapacity * cfg_sim['bat_maxcrate'] * -1000))
            exit()
        if self.batpmax > self.batcapacity * cfg_sim['bat_maxcrate'] * 1000:
            print("Input for battery " + str(
                self.number) + " is invalid! The maximumpower (batpmax) may not be higher than -" + str(
                cfg_sim['bat_maxcrate']) + " C-rate. The highest allowed value given its capacity is: " + str(
                self.batcapacity * cfg_sim['bat_maxcrate'] * 1000))
            exit()
